...
const tabMeta: Record<TabId, { title: string, description: string, badge: string, accent: string, accentMuted: string, accentBorder: string }> = {
  regex: {
    title: 'Pattern Tester',
    description: 'Validate patterns, inspect groups, and preview matches inline as you edit.',
    badge: 'Live Regex',
    accent: 'var(--pd-accent)',
    accentMuted: 'color-mix(in srgb, var(--pd-accent) 12%, transparent)',
    accentBorder: 'color-mix(in srgb, var(--pd-accent) 30%, var(--pd-border))',
  },
  timestamp: {
    title: 'Time Converter',
    description: 'Jump between Unix timestamps and readable dates with instant local and UTC output.',
    badge: 'Date + Time',
    accent: '#0f766e',
  ...
  color: {
    title: 'Color Lab',
    description: 'Convert HEX, RGB, and HSL values while previewing the final swatch in real time.',
    badge: 'Visual Picker',
    accent: '#c2410c',
    accentMuted: 'rgba(194, 65, 12, 0.12)',
    accentBorder: 'rgba(194, 65, 12, 0.32)',
  },
  diff: {
    title: 'Text Compare',
    description: 'Paste two versions of text and inspect additions, removals, and unchanged lines clearly.',
    badge: 'Line Diff',
    accent: '#7c3aed',
...
const activeTabStyle = computed(() => ({
  '--dt-accent': activeTabMeta.value.accent,
  '--dt-accent-muted': activeTabMeta.value.accentMuted,
  '--dt-accent-border': activeTabMeta.value.accentBorder,
}))
...
const activeTabStats = computed(() => {
  switch (activeTab.value) {
    case 'regex':
      return [
        { label: 'Matches', value: String(regexMatches.value.length) },
        { label: 'Flags', value: regexFlags.value || 'none' },
      ]
    case 'timestamp':
      return [
        { label: 'Unit', value: timestampUnit.value },
        { label: 'Now', value: now.value.format('HH:mm:ss') },
      ]
    case 'color':
      return [
        { label: 'HEX', value: colorHex.value.toUpperCase() },
        { label: 'Hue', value: `${colorH.value}deg` },
      ]
    case 'diff':
      return [
        { label: 'Adds', value: `+${diffStats.value.adds}` },
        { label: 'Removes', value: `-${diffStats.value.removes}` },
      ]
    default:
      return []
  }
...
function isActiveTab(tabId: TabId) {
...
<template>
  ...
  >
    <!-- Header -->
    <header class="px-3 border-b border-pd-border bg-pd-bg/88 flex shrink-0 h-12 items-center top-0 justify-between sticky z-50 backdrop-blur-md md:px-5">
      <div class="flex gap-2 min-w-0 items-center md:gap-3">
        <router-link
          to="/"
        ...
        >
          <ArrowLeft
            ...
            :stroke-width="1.5"
          />
          <span class="text-[10px] tracking-[0.22em] font-bold hidden uppercase md:inline">Back</span>
        </router-link>
        <div class="bg-pd-border h-4 w-px" />
        <div class="flex gap-2 items-center">
          <div class="p-1 border border-pd-accent/20 rounded-[2px] bg-pd-accent-muted">
            <Code2 class="text-pd-accent h-3.5 w-3.5" :stroke-width="1.5" />
          </div>
          <span class="text-xs tracking-[0.22em] font-bold uppercase md:text-sm">
      ...
      <div class="text-xs text-pd-text-muted flex shrink-0 gap-2 items-center md:gap-3">
        <button
          class="text-[9px] tracking-[0.18em] font-bold px-2.5 py-1 border border-pd-border rounded-sm flex gap-1.5 uppercase transition-all items-center hover:text-pd-danger hover:border-pd-danger/50"
          @click="clearAll"
        >
    ...
    <main class="flex flex-1 flex-col min-h-0">
      <section class="px-3 py-2 border-b border-pd-border bg-pd-bg-subtle/12 shrink-0 md:px-5">
        <div class="flex flex-wrap gap-2 items-center justify-between">
          <div class="flex flex-wrap gap-x-3 gap-y-1 min-w-0 items-center">
            <h1 class="text-[11px] text-pd-text tracking-[0.22em] font-bold uppercase">
              {{ activeTabMeta.title }}
            </h1>
            <span class="text-[9px] tracking-[0.18em] font-bold uppercase" style="color: var(--dt-accent)">
              {{ activeTabMeta.badge }}
            </span>
            <div class="flex-wrap gap-2 hidden items-center md:flex">
              <div
                v-for="item in activeTabStats"
                :key="item.label"
                class="text-[9px] tracking-[0.12em] px-2 py-0.75 border border-pd-border rounded-sm bg-pd-bg uppercase"
                style="border-color: var(--dt-accent-border)"
              >
                <span class="text-pd-text-muted">{{ item.label }}:</span>
                <span class="text-pd-text font-bold ml-1">{{ item.value }}</span>
              </div>
            </div>
          </div>

          <div class="gap-2 grid grid-cols-2 items-center sm:flex">
            <button
              v-for="tab in tabs"
              :key="tab.id"
              class="text-[9px] tracking-[0.14em] font-bold px-2.5 py-1.5 border rounded-sm flex gap-1.5 whitespace-nowrap uppercase transition-all items-center justify-center"
      ...
      <section class="flex-1 min-h-0 overflow-hidden">
        <!-- ═══ REGEX TAB ═══ -->
        <div
          v-if="isActiveTab('regex')"
          class="flex flex-col h-full overflow-hidden"
        >
          <!-- Pattern Input -->
          <div class="px-4 py-3 border-b border-pd-border bg-pd-bg-subtle/20 flex shrink-0 flex-wrap gap-3 items-center md:px-6">
          ...
          <div class="flex flex-1 min-h-0" :class="isDesktop ? 'flex-row overflow-hidden' : 'flex-col overflow-y-auto'">
            <!-- Test String -->
            <div
              class="border-r border-pd-border flex flex-1 flex-col min-h-[50vh] min-w-0"
              :class="isDesktop ? 'border-b-0 min-h-0' : 'border-b'"
            >
              <div class="px-4 py-2.5 border-b border-pd-border bg-pd-bg-subtle/20 flex shrink-0 gap-3 items-center justify-between md:px-6">
              ...
              <textarea
                ...
                class="text-sm text-pd-text leading-relaxed font-mono p-4 outline-none bg-pd-bg flex-1 min-h-[320px] resize-none md:p-6 placeholder-pd-text-disabled"
            ...
            <div class="flex flex-1 flex-col min-h-[50vh] min-w-0" :class="isDesktop ? 'min-h-0' : ''">
              <div class="px-4 py-2.5 border-b border-pd-border bg-pd-bg-subtle/20 shrink-0 md:px-6">
                <span class="text-[9px] text-pd-text-muted tracking-[0.2em] font-bold uppercase">
                  MATCH RESULTS
                </span>
              </div>
              <div class="custom-scrollbar p-4 flex-1 overflow-y-auto md:p-6">
                <!-- Highlighted Preview -->
        ...
        <!-- ═══ TIMESTAMP TAB ═══ -->
        <div
          v-if="isActiveTab('timestamp')"
          class="h-full overflow-hidden"
        >
          <div class="custom-scrollbar p-3 h-full overflow-y-auto md:p-4">
            <div class="mx-auto flex flex-col gap-4 max-w-6xl md:gap-5">
              <!-- Current Info Banner -->
              <div class="gap-3 grid grid-cols-1 md:grid-cols-3">
                <div class="p-4 border border-pd-border rounded-sm bg-pd-bg-subtle/10 flex flex-col gap-2 shadow-sm">
                ...
                <div class="p-4 border border-pd-border rounded-sm bg-pd-bg-subtle/10 flex flex-col gap-2 col-span-1 shadow-sm md:col-span-2">
              ...
              <div class="px-1 flex flex-wrap gap-3 items-center md:gap-6">
                <div class="flex flex-wrap gap-4 items-center">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button
                      ...
                      :key="unit"
                      class="text-[10px] tracking-widest font-bold px-3 py-1.5 border rounded-sm uppercase transition-all"
                ...
                <div class="bg-pd-border/60 h-4 w-px hidden md:block" />

                <button
                  class="text-[10px] text-pd-text-muted tracking-widest font-bold px-3 py-1.5 border border-pd-border rounded-sm flex gap-2 uppercase transition-all items-center hover:text-pd-accent hover:border-pd-accent/50"
                  @click="fillNow"
                >
              ...
              <div class="gap-6 grid grid-cols-1 xl:grid-cols-2">
                <!-- Timestamp → Date -->
                <div class="p-4 border border-pd-border rounded-sm bg-pd-bg flex flex-col gap-5 shadow-sm md:p-5">
                  <div class="flex flex-wrap gap-3 items-center justify-between">
                    ...
                    <button
                      class="text-[9px] text-pd-accent tracking-widest font-bold px-2.5 py-1 border border-pd-border rounded-sm flex gap-2 uppercase transition-all items-center hover:border-pd-accent/50 hover:bg-pd-accent-muted"
                  ...
                  <div class="relative">
                    <input
                      ...
                      class="text-sm text-pd-text font-mono p-4 pr-10 outline-none border-b border-pd-border bg-transparent w-full transition-colors focus:border-pd-accent"
                      placeholder="Enter timestamp (e.g. 1709827200)"
                    >
                    <Clock class="text-pd-text-muted h-4 w-4 right-3 top-1/2 absolute -translate-y-1/2" />
                  </div>

                  <div class="p-4 border border-pd-border rounded-sm bg-pd-bg-subtle/10 flex min-h-28 items-center justify-center">
                    <pre
                ...
                <div class="p-4 border border-pd-border rounded-sm bg-pd-bg flex flex-col gap-5 shadow-sm md:p-5">
                  ...
                  <div class="relative">
                    ...
                    >
                    <Calendar class="text-pd-text-muted h-4 w-4 right-3 top-1/2 absolute -translate-y-1/2" />
                  </div>
        ...
        <!-- ═══ COLOR TAB ═══ -->
        <div
          v-if="isActiveTab('color')"
          class="h-full overflow-hidden"
        >
          <div class="custom-scrollbar p-3 h-full overflow-y-auto md:p-4">
            <div class="mx-auto max-w-6xl">
              <!-- Color Preview -->
              <div class="mb-4 flex flex-col gap-4 items-start xl:flex-row">
                <div
                  class="border border-pd-border rounded-sm shrink-0 h-32 w-full shadow-xl xl:h-44 xl:w-44"
              ...
              <!-- Quick Copy Formats -->
              <div class="gap-3 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4">
                <button
                ...
                >
                  <div>
                    ...
                    <p class="text-xs text-pd-text font-bold font-mono mt-1 break-all">
                      {{ fmt.value }}
                    </p>
                  </div>
        ...
        <!-- ═══ DIFF TAB ═══ -->
        <div
          v-if="isActiveTab('diff')"
          class="flex flex-col h-full overflow-hidden"
        >
          <!-- Stats Bar -->
          <div class="px-4 py-2.5 border-b border-pd-border bg-pd-bg-subtle/20 flex shrink-0 flex-wrap gap-4 items-center md:px-6">
            <span class="text-[9px] text-pd-text-muted tracking-[0.2em] font-bold uppercase">
              DIFF STATS
            </span>
          ...
          <div class="flex flex-1 min-h-0" :class="isDesktop ? 'flex-row overflow-hidden' : 'flex-col overflow-y-auto'">
            <!-- Left: Text A -->
            <div
              class="border-r border-pd-border flex flex-1 flex-col min-h-[33vh] min-w-0 overflow-auto"
              :class="isDesktop ? 'border-b-0 min-h-0 md:overflow-hidden' : 'border-b'"
            >
              <div class="px-4 py-2.5 border-b border-pd-border bg-pd-bg-subtle/20 flex shrink-0 gap-3 items-center justify-between md:px-6">
                <span class="text-[9px] text-pd-text-muted tracking-[0.2em] font-bold uppercase">
                  ORIGINAL (A)
              ...
              <textarea
                v-model="diffTextA"
                class="text-sm text-pd-text leading-relaxed font-mono p-4 outline-none bg-pd-bg flex-1 min-h-[240px] resize-none md:p-6 placeholder-pd-text-disabled"
                :class="isDesktop ? 'min-h-0' : ''"
                placeholder="粘贴原始文本..."
                spellcheck="false"
              />
            ...
            <!-- Center: Diff Result -->
            <div
              class="border-b flex flex-1 flex-col min-h-[33vh] min-w-0 overflow-auto"
              :class="isDesktop ? 'border-b-0 min-h-0 md:overflow-hidden' : ''"
            >
              <div class="px-4 py-2.5 border-b border-pd-border bg-pd-bg-subtle/20 shrink-0 md:px-6">
                <span class="text-[9px] text-pd-text-muted tracking-[0.2em] font-bold uppercase">
                  DIFF RESULT
            ...
            <!-- Right: Text B -->
            <div
              class="border-l border-pd-border flex flex-1 flex-col min-h-[33vh] min-w-0 overflow-auto"
              :class="isDesktop ? 'min-h-0 md:overflow-hidden' : 'border-l-0'"
            >
              <div class="px-4 py-2.5 border-b border-pd-border bg-pd-bg-subtle/20 flex shrink-0 gap-3 items-center justify-between md:px-6">
                <span class="text-[9px] text-pd-text-muted tracking-[0.2em] font-bold uppercase">
                  MODIFIED (B)
        ...
        <div
          v-if="errorMessage"
          class="px-4 py-2 border-t border-pd-danger/20 bg-pd-danger-muted flex shrink-0 gap-2 items-center md:px-6"
        >
        ...
        <!-- Status Bar -->
        <div class="px-4 py-2 border-t border-pd-border bg-pd-bg-subtle/20 flex shrink-0 flex-wrap gap-2 items-center justify-between md:px-6">
